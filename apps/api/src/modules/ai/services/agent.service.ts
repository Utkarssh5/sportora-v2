...
function detectTournamentIntent(prompt: string): {
  isTournamentIntent: boolean;
  ...
  const sport = Object.entries(sports).find(
    ([key]) => text.includes(key)
  )?.[1];

  const hasTournamentWord =
    tournamentWords.some((word) => text.includes(word));

  const hasDiscoveryWord =
    discoveryWords.some((word) => text.includes(word));
  ...
  return {
    isTournamentIntent:
      hasTournamentWord ||
      (hasSport && hasDiscoveryWord) ||
      (hasSport && text.length < 80),
    sport,
  };
}

function isNaturalConfirmation(prompt: string): boolean {
  const text = prompt
    .trim()
    .toLowerCase()
    .replace(/[!?.,]/g, "")
...
function isNaturalNegative(prompt: string): boolean {
  const text = prompt
    .trim()
    .toLowerCase()
    .replace(/[!?.,]/g, "")
  ...
  return negatives.includes(text);
}
  ...
  public static async chat(
    ...
    for (
      ...
      try {
        ...
        if (temporaryFailure) {
          const fallback =
            buildTemporaryAIResponse(prompt);
...
function buildTemporaryAIResponse(prompt: string): string {
  const text = prompt.trim().toLowerCase();

  if (
    /^(hi|hello|hey|hii|hlo)\b/.test(text) ||
