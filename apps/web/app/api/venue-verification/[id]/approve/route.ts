...
export async function PATCH(
  ...
  try {
    ...
    const authResult = await authenticatedFetch(
      `/api/v1/venue-verification/${id}/approve`,
      accessToken,
      refreshToken,
      {
        method: 'PATCH',
      ...
      return NextResponse.json(
        {
          ...
          error:
            data.message ||
            data.error ||
            'Failed to approve venue verification.',
        },
    ...
    const result = NextResponse.json({
      success: true,
      message: data.message || 'Venue verification approved.',
      data: data.data,
    });
    ...
    console.error('Admin venue approval proxy error:', error);
