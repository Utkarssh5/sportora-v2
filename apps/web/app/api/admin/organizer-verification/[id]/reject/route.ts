  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { success: false, error: data.message || data.error || 'Failed to reject organizer.' },
    ...
    const result = NextResponse.json({
      success: true,
      message: data.message || 'Organizer rejected.',
      data: data.data,
    });
    ...
    console.error('Admin organizer rejection proxy error:', error);
    return NextResponse.json(
