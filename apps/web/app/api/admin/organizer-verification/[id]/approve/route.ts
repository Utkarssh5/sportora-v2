  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { success: false, error: data.message || data.error || 'Failed to approve organizer.' },
    ...
    const result = NextResponse.json({
      success: true,
      message: data.message || 'Organizer approved.',
      data: data.data,
    });
    ...
    console.error('Admin organizer approval proxy error:', error);
    return NextResponse.json(
